...
// FIREBASE & AUTH
// 👇 db importunu ekledik
import { auth, db } from "../../firebase";
import { useAuth } from "../../context/AuthContext";
// 👇 Firestore sorgu fonksiyonlarını ekledik
...
const StoreDashboard = () => {
    ...
    useEffect(() => {
        if (id) {
            ...
            // 2. BUGÜNÜN Cirosunu Çek (Gerçek Veri)
    ...
    const fetchDailyTurnover = async (storeId: string) => {
        try {
            // Bugünün tarihini YYYY-MM-DD formatında al (Yerel Saat ile)
            const now = new Date();
            const year = now.getFullYear();
            const month = String(now.getMonth() + 1).padStart(2, '0');
            const day = String(now.getDate()).padStart(2, '0');
            const todayString = `${year}-${month}-${day}`; // Örn: "2026-01-21"

            // Sorgu: Bu mağazanın, tarihi "bugün" olan fişleri
            const q = query(
            ...
            console.error("Günlük ciro çekilemedi:", error);
        }
    };
                        ...
                        {viewMode === 'balance' ? (
                            ...
                            >
                                <div style={{ fontSize: '42px', fontWeight: '800', letterSpacing: '-1px', color: '#38bdf8', marginBottom: '5px' }}>
                                    {/* 👇 ARTIK GERÇEK CİROYU GÖSTERİYOR */}
                                ...
                                <div style={{ display: 'flex', gap: '15px', fontSize: '13px', opacity: 0.8 }}>
                                    ...
                                    <span>•</span>
                                    <span>{dailyStats.count} İşlem</span>
                                </div>
                            </motion.div>
                        )}
                    </AnimatePresence>
